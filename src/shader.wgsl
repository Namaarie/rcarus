...
struct VertexOutput {
    // @builtin(position) tells wgpu that clip_position is the built in for position
    // @builtin(position) is in framebuffer space 
    // ranges from 0-screen_width and 0-screen_height
    ...
    @location(7) vertex_color: vec3<f32>,
};
...
@fragment
// @location(0) tells wgpu to store the vec4 value in the first color target
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    let vertex_color = in.vertex_color;
    let object_color: vec4<f32> = textureSample(t_diffuse, s_diffuse, in.tex_coords) * vec4<f32>(vertex_color, 1.0);
